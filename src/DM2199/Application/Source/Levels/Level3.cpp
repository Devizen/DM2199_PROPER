...
Inventory* level3_inventory = 0;

void Level3::loadConfig()
{
...
void Level3::Init()
{
    ...
    if (level3_inventory == 0)
    {
        level3_inventory = new Inventory();
        level3_inventory->setdefined(1);
    }
    //type , damage , range , reload speed , attack speed
    ItemInfo* gun = new ItemInfo("gun", 20, 20, 10, 15);
    level3_inventory->addItem(gun);

    ItemInfo* torch = new ItemInfo("torch", 0, 50, 0, 10);
    level3_inventory->addItem(torch);
    // adding items to inventory
    ItemInfo* fist = new ItemInfo();
    level3_inventory->addItem(fist);
...
void Level3::Update(double dt)
{
    ...
    Camera3::collisionSwitch(true, "Level3.txt");
...
void Level3::enemyUpdate(double dt)
{
    //-------This function is called in Level3::Update(double dt) to process Enemy interactions-----------------
...
void Level3::renderInventory()
{
    if (!level3_inventory->pointer)
    {
    ...
    ItemInfo* activeItem = level3_inventory->getActiveItem();
    ...
    ItemInfo* secondaryItem = level3_inventory->getSecondaryItem();
    ...
    ItemInfo* ItemDisplay1 = level3_inventory->getDisplay1();
    ...
    ItemInfo* ItemDisplay2 = level3_inventory->getDisplay2();
...
void Level3::renderElectricField()
{
...
void Level3::renderSprites()
{
    ...
    if (level3_inventory->getActiveItem()->gettype() == "fist" || level3_inventory->getSecondaryItem()->gettype() == "fist")
    {
        if (swing == false)
        {
    ...
    else if (level3_inventory->getActiveItem()->gettype() == "gun" || level3_inventory->getSecondaryItem()->gettype() == "gun")
    {
        if (swing == false)
        {
    ...
    else if (level3_inventory->getActiveItem()->gettype() == "torch" || level3_inventory->getSecondaryItem()->gettype() == "torch")
        RenderMeshOnScreen(meshList[GEO_TORCH], 25, 5, 150, 150, 0.f, 0.f, 0.f, 1.f);
}


void Level3::renderPosition()
{
...
void Level3::renderRemainingTime()
{
...
void Level3::renderAntidoteCount()
{
    std::string count = "";
...
void Level3::renderStats()
{
    /*ItemInfo* active = level3_inventory->getActiveItem();
    string itemName = active->gettype();
    RenderTextOnScreen(meshList[GEO_FPS], itemName, Color(0,0,0), 2, 25, 20);*/
}

void Level3::RenderMesh(Mesh *mesh, bool enableLight) {
...
void Level3::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void Level3::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
    if (!mesh || mesh->textureID <= 0) //Proper error check
    {
        return;
...
void Level3::RenderMeshOnScreen(Mesh* mesh, float x, float y, float sizex, float sizey, float rotateAngle, float xAxis, float yAxis, float zAxis)
{
    glDisable(GL_DEPTH_TEST);
    Mtx44 ortho;
    ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
