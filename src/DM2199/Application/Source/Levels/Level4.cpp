...
Inventory* level4_inventory = 0;

void Level4::loadConfig()
{
...
void Level4::Init()
{
    ...
    light[1].position.Set(-93.f, 0.f, -2.f);
    ...
    light[2].position.Set(468.f, 0.f, 248.f);
    light[2].color.Set(1.f, 0.f, 0.f);
    light[2].power = 1.f;
    light[2].kC = 1.f;
    light[2].kL = 0.01f;
    light[2].kQ = 0.001f;
    ...
    light[3].position.Set(-93.f, 0.f, -2.f);
    light[3].color.Set(1.f, 0.f, 0.f);
    light[3].power = 1.f;
    light[3].kC = 1.f;
    light[3].kL = 0.01f;
    light[3].kQ = 0.001f;
    ...
    if (level4_inventory == 0)
    {
        level4_inventory = new Inventory();
        level4_inventory->setdefined(1);
    }
    //type , damage , range , reload speed , attack speed
    ItemInfo* gun = new ItemInfo("gun", 20, 20, 10, 15);
    level4_inventory->addItem(gun);

    ItemInfo* torch = new ItemInfo("torch", 0, 50, 0, 10);
    level4_inventory->addItem(torch);
    // adding items to inventory
    ItemInfo* fist = new ItemInfo();
    level4_inventory->addItem(fist);
...
void Level4::Update(double dt)
{
    ...
    Camera3::collisionSwitch(true, "Level4.txt");
    ...
    if (canPress && Application::IsKeyPressed(MK_RBUTTON)) {
        light[0].power = (light[0].power > 0) ? 0.0f : 10.0f; // Toggle Power between 0 and 2
        glUniform1f(m_parameters[U_LIGHT0_POWER], light[0].power);
        canPress = false;
    }
    if (light[0].power > 0)
    ...
    if (camera.health <= 0 || timeleft <= 0)
    {
        Application::ChangeScene(10);
    }
}

void Level4::enemyUpdate(double dt)
{
	//-------This function is called in Level4::Update(double dt) to process Enemy interactions-----------------
	...
	for (vector<Enemy*>::iterator it = enemyStorage.begin(); it != enemyStorage.end(); it++)
	{
		...
		/*		if (((*factoryIt)->position_ - (*it)->_Position).Length() < 20 && bulletTouch == false)
				{*/
                if ((((*factoryIt)->position_.x >= (*it)->_Position.x - 50 && (*factoryIt)->position_.x <= (*it)->_Position.x + 50) &&
                    ((*factoryIt)->position_.y >= (*it)->_Position.y - 30 && (*factoryIt)->position_.y <= (*it)->_Position.y + 30) &&
                    ((*factoryIt)->position_.z >= (*it)->_Position.z - 50 && (*factoryIt)->position_.z <= (*it)->_Position.z + 50)) &&
                    bulletTouch == false)
                {
...
void Level4::renderHUD()
{
    ...
    else
    {
        ...
        if (camera.health > 0)
        {
            RenderMeshOnScreen(meshList[GEO_HP], 5 + 11.3 * camera.health / 500, 6.1, 105 * camera.health / 500, 9, 0.f, 0.f, 0.f, 1.f);
        }
        ...
        if (energy > 0)
        {
            RenderMeshOnScreen(meshList[GEO_EN], 5 + 11.3* energy / 1000, 2.3, 105 * energy / 1000, 9, 0.f, 0.f, 0.f, 1.f);
        }
        if (energy == 0)
        {
            torchDead = true;
        ...
        if (timeleft > 0)
        {
            RenderMeshOnScreen(meshList[GEO_TIME], 58.6 + 11.3* timeleft / 60000, 14.5, 105 * timeleft / 60000, 11, 0.f, 0.f, 0.f, 1.f);
        }
        renderRemainingTime();
        //minimap
...
void Level4::renderInventory()
{
    if (!level4_inventory->pointer)
    {
    ...
    ItemInfo* activeItem = level4_inventory->getActiveItem();
    ...
    ItemInfo* secondaryItem = level4_inventory->getSecondaryItem();
    ...
    ItemInfo* ItemDisplay1 = level4_inventory->getDisplay1();
    ...
    {
        RenderMeshOnScreen(meshList[GEO_ACTIVE_SELECT], 12, 23.2, 6, 7.2, 0.f, 0.f, 0.f, 1.f);
    }
    else if (ItemDisplay1 == secondaryItem)
    {
        RenderMeshOnScreen(meshList[GEO_SECONDARY_SELECT], 12, 23.2, 6, 7.2, 0.f, 0.f, 0.f, 1.f);
    }
    ItemInfo* ItemDisplay2 = level4_inventory->getDisplay2();
    ...
    {
        RenderMeshOnScreen(meshList[GEO_ACTIVE_SELECT], 12, 13.7, 6, 7.2, 0.f, 0.f, 0.f, 1.f);
    }
    else if (ItemDisplay2 == secondaryItem)
    {
        RenderMeshOnScreen(meshList[GEO_SECONDARY_SELECT], 12, 13.7, 6, 7.2, 0.f, 0.f, 0.f, 1.f);
    }
...
void Level4::renderElectricField()
{
    //----------------------------------Electric Field---------------------------------------------------------------
    //Check if player is stepping on electric field healing pad.
    if (camera.position.x > -30 && camera.position.x < 30 && camera.position.z > -30 && camera.position.z < 30 ||
        camera.position.x > -255 && camera.position.x < -195 && camera.position.z > -210 && camera.position.z < -150 ||
        camera.position.x > 370 && camera.position.x < 430 && camera.position.z > 50 && camera.position.z < 110)
    {
        energy += 3;
        if (camera.position.x > -30 && camera.position.x < 30 && camera.position.z > -30 && camera.position.z < 30)
        {
    ...
    modelStack.PushMatrix();
    modelStack.Translate(-225.f, -30.f, -180.f);
    modelStack.Rotate(90.f, 1.f, 0.f, 0.f);
    modelStack.Scale(10.f, 10.f, 10.f);
    RenderMesh(meshList[GEO_ELECTRIC_FIELD], true);
    modelStack.PopMatrix();
    ...
    modelStack.PushMatrix();
    modelStack.Translate(400.f, -30.f, 80.f);
    modelStack.Rotate(90.f, 1.f, 0.f, 0.f);
    modelStack.Scale(10.f, 10.f, 10.f);
    RenderMesh(meshList[GEO_ELECTRIC_FIELD], true);
    modelStack.PopMatrix();
    ...
    if (camera.position.x > 480.f && camera.position.x < 520.f && camera.position.z > -390.f && camera.position.z < -350.f)
    {
    ...
    modelStack.PushMatrix();
    modelStack.Translate(500.f, -30.f, -370.f);
    modelStack.Rotate(90.f, 1.f, 0.f, 0.f);
    modelStack.Scale(20.f, 20.f, 20.f);
    RenderMesh(meshList[GEO_PORTAL], true);
    modelStack.PopMatrix();
    //-------------------------------------End Portal to next level----------------------------------------
}

void Level4::RenderSkybox()
{
    //Day
    if (cycle == false)
    {
...
void Level4::renderSprites()
{
    ...
    if (level4_inventory->getActiveItem()->gettype() == "fist" || level4_inventory->getSecondaryItem()->gettype() == "fist")
    {
        if (swing == false)
        {
    ...
    else if (level4_inventory->getActiveItem()->gettype() == "gun" || level4_inventory->getSecondaryItem()->gettype() == "gun")
    {
        if (swing == false)
        {
    ...
    else if (level4_inventory->getActiveItem()->gettype() == "torch" || level4_inventory->getSecondaryItem()->gettype() == "torch")
        RenderMeshOnScreen(meshList[GEO_TORCH], 25, 5, 150, 150, 0.f, 0.f, 0.f, 1.f);
}


void Level4::renderPosition()
{
    int enemiesCount = 0;
    for (vector<Enemy*>::iterator it = enemyStorage.begin(); it != enemyStorage.end(); it++)
    {
        if ((*it)->getState() != Spider::spiderState::death || (*it)->getState() != Robot::robotState::death)
        {
            enemiesCount++;
        }
    }
    ...
    string enemiesLeft = "Enemies Left: " + to_string((int)enemiesCount);
    ...
    modelStack.PushMatrix();
    RenderTextOnScreen(meshList[GEO_X], enemiesLeft, Color(0, 1, 0), 3, 0, 3);
    modelStack.PopMatrix();
}


void Level4::renderText()
{
...
void Level4::renderRemainingTime()
{
...
void Level4::renderAntidoteCount()
{
    std::string count = "";
...
void Level4::renderStats()
{
    /*ItemInfo* active = level4_inventory->getActiveItem();
    string itemName = active->gettype();
    RenderTextOnScreen(meshList[GEO_FPS], itemName, Color(0,0,0), 2, 25, 20);*/
}


void Level4::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
...
void Level4::RenderMesh(Mesh *mesh, bool enableLight) {
...
void Level4::RenderMeshOnScreen(Mesh* mesh, float x, float y, float sizex, float sizey, float rotateAngle, float xAxis, float yAxis, float zAxis)
{
    glDisable(GL_DEPTH_TEST);
    Mtx44 ortho;
    ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
...
void Level4::resetAll()
{
    camera.position.Set(500.f, 0.f, -370.f);
}
