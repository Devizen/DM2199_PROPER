...
Inventory* boss_inventory = 0;

void Boss::loadConfig()
{
...
void Boss::Init()
{
    ...
    //Enemy * newEnemy = new Spider(70.f, 5.f, 50.f, 100.f ,1.f);
    ...
    //Enemy * newEnemy3 = new Spider(70.f, 5.f, 50.f, 100.f, 1.f);
...
void Boss::enemyUpdate(double dt)
{
    //-------This function is called in Boss::Update(double dt) to process Enemy interactions-----------------
...
void Boss::renderInventory()
{
    if (!boss_inventory->pointer)
    {
    ...
    ItemInfo* activeItem = boss_inventory->getActiveItem();
    ...
    ItemInfo* secondaryItem = boss_inventory->getSecondaryItem();
    ...
    ItemInfo* ItemDisplay1 = boss_inventory->getDisplay1();
    ...
    ItemInfo* ItemDisplay2 = boss_inventory->getDisplay2();
...
void Boss::renderSprites()
{
    ...
    if (boss_inventory->getActiveItem()->gettype() == "fist" || boss_inventory->getSecondaryItem()->gettype() == "fist")
    {
        if (swing == false)
        {
    ...
    else if (boss_inventory->getActiveItem()->gettype() == "gun" || boss_inventory->getSecondaryItem()->gettype() == "gun")
    {
        if (swing == false)
        {
    ...
    else if (boss_inventory->getActiveItem()->gettype() == "torch" || boss_inventory->getSecondaryItem()->gettype() == "torch")
        RenderMeshOnScreen(meshList[GEO_TORCH], 25, 5, 150, 150, 0.f, 0.f, 0.f, 1.f);
}


void Boss::renderPosition()
{
...
void Boss::renderStats()
{
    /*ItemInfo* active = boss_inventory->getActiveItem();
    string itemName = active->gettype();
    RenderTextOnScreen(meshList[GEO_FPS], itemName, Color(0,0,0), 2, 25, 20);*/
}

void Boss::RenderMesh(Mesh *mesh, bool enableLight) {
...
void Boss::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void Boss::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
    if (!mesh || mesh->textureID <= 0) //Proper error check
    {
        return;
...
void Boss::RenderMeshOnScreen(Mesh* mesh, float x, float y, float sizex, float sizey, float rotateAngle, float xAxis, float yAxis, float zAxis)
{
    glDisable(GL_DEPTH_TEST);
    Mtx44 ortho;
    ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
