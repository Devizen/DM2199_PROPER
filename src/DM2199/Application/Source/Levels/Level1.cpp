...
void Level1::Init()
{
    ...
    if (level1_inventory == 0)
    {
        level1_inventory = new Inventory();
        level1_inventory->setdefined(1);
    }
    //type , damage , range , reload speed , attack speed
    ItemInfo* gun = new ItemInfo("gun", 20, 20, 10, 15);
    level1_inventory->addItem(gun);

    ItemInfo* torch = new ItemInfo("torch", 0, 50, 0, 10);
    level1_inventory->addItem(torch);
    // adding items to inventory
    ItemInfo* fist = new ItemInfo();
    level1_inventory->addItem(fist);
...
void Level1::enemyUpdate(double dt)
{
    //-------This function is called in Level1::Update(double dt) to process Enemy interactions-----------------
...
void Level1::renderInventory()
{
    if (!level1_inventory->pointer)
    {
    ...
    ItemInfo* activeItem = level1_inventory->getActiveItem();
    ...
    ItemInfo* secondaryItem = level1_inventory->getSecondaryItem();
    ...
    ItemInfo* ItemDisplay1 = level1_inventory->getDisplay1();
    ...
    ItemInfo* ItemDisplay2 = level1_inventory->getDisplay2();
...
void Level1::renderElectricField()
{
...
void Level1::renderSprites()
{
    ...
    if (level1_inventory->getActiveItem()->gettype() == "fist" || level1_inventory->getSecondaryItem()->gettype() == "fist")
    {
        if (swing == false)
        {
    ...
    else if (level1_inventory->getActiveItem()->gettype() == "gun" || level1_inventory->getSecondaryItem()->gettype() == "gun")
    {
        if (swing == false)
        {
    ...
    else if (level1_inventory->getActiveItem()->gettype() == "torch" || level1_inventory->getSecondaryItem()->gettype() == "torch")
        RenderMeshOnScreen(meshList[GEO_TORCH], 25, 5, 150, 150, 0.f, 0.f, 0.f, 1.f);
}


void Level1::renderPosition()
{
...
void Level1::renderRemainingTime()
{
...
void Level1::renderAntidoteCount()
{
    std::string count = "";
...
void Level1::renderStats()
{
    /*ItemInfo* active = level1_inventory->getActiveItem();
    string itemName = active->gettype();
    RenderTextOnScreen(meshList[GEO_FPS], itemName, Color(0,0,0), 2, 25, 20);*/
}

void Level1::RenderMesh(Mesh *mesh, bool enableLight) {
...
void Level1::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void Level1::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
    if (!mesh || mesh->textureID <= 0) //Proper error check
    {
        return;
...
void Level1::RenderMeshOnScreen(Mesh* mesh, float x, float y, float sizex, float sizey, float rotateAngle, float xAxis, float yAxis, float zAxis)
{
    glDisable(GL_DEPTH_TEST);
    Mtx44 ortho;
    ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
