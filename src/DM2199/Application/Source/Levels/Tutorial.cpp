...
void Tutorial::Init()
{
    ...
    meshList[GEO_CURRENT_SELECT] = MeshBuilder::GenerateOBJ("current_select", "OBJ//inventory.obj");
    meshList[GEO_CURRENT_SELECT]->textureID = LoadTGA("Image//current_select.tga");
    //Prevent Jerk
...
void Tutorial::Update(double dt)
{
    ...
    Camera3::collisionSwitch(true, "Tutorial.txt");
...
void Tutorial::enemyUpdate(double dt)
{
...
void Tutorial::renderHUD()
{
    if (inventoryOpen)
    {
        if (startTime >= cooldown)
        {
            ...
            {
                global_inventory->sidepointer = false;
                startTime = 0;
            }
            else if (Application::IsKeyPressed(VK_RIGHT))
            {
                global_inventory->sidepointer = true;
                startTime = 0;
            }
            else if (Application::IsKeyPressed(VK_RETURN))
            {
                if (global_inventory->sidepointer)
                {
    ...
    else
    {
        ...
        if (camera.health > 0)
        {
            RenderMeshOnScreen(meshList[GEO_HP], 5 + 11.3 * camera.health / camera.maxhealth, 6.1, 105 * camera.health / camera.maxhealth, 9, 0.f, 0.f, 0.f, 1.f);
        }
        ...
        if (energy > 0)
        {
            RenderMeshOnScreen(meshList[GEO_EN], 5 + 11.3* energy / maxenergy, 2.3, 105 * energy / maxenergy, 9, 0.f, 0.f, 0.f, 1.f);
        }
        if (energy == 0)
        {
            torchDead = true;
        ...
        if (timeleft > 0)
        {
            RenderMeshOnScreen(meshList[GEO_TIME], 58.6 + 11.3* timeleft / maxtime, 14.5, 105 * timeleft / maxtime, 11, 0.f, 0.f, 0.f, 1.f);
        }
        renderRemainingTime();
        //minimap
...
void Tutorial::renderInventory()
{
    ...
    if (!global_inventory->sidepointer)
    {
        RenderMeshOnScreen(meshList[GEO_CURRENT_SELECT], 14.9, 36.5, 11.2, 14, 0.f, 0.f, 0.f, 1.f);
    }
    else
    {
        RenderMeshOnScreen(meshList[GEO_CURRENT_SELECT], 32.5, 36.5, 11.2, 14, 0.f, 0.f, 0.f, 1.f);
    }
...
void Tutorial::renderRemainingTime()
{
...
void Tutorial::renderObjective()
{
    ...
    string current = "Kill all enemies";
    string enemyCount = "Enemies Remaining: " + to_string(eCount);
    RenderTextOnScreen(meshList[GEO_COUNTDOWN], Objective, Color(0, 0, 0), 2, 28, 5.5);
    RenderTextOnScreen(meshList[GEO_COUNTDOWN], current, Color(0, 0, 0), 2, 28, 5);
    RenderTextOnScreen(meshList[GEO_COUNTDOWN], enemyCount, Color(0, 0, 0), 2, 28, 4);
}
void Tutorial::renderPotionCount()
{
    std::string count = "";
...
void Tutorial::renderAntidoteCount()
{
    std::string count = "";
...
void Tutorial::RenderMesh(Mesh *mesh, bool enableLight) {
...
void Tutorial::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void Tutorial::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
    if (!mesh || mesh->textureID <= 0) //Proper error check
    {
        return;
