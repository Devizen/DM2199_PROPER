...
void SceneLose::setRetry(unsigned input)
{
    retryLevel = input;
}
void SceneLose::Init()
{
	...
	meshList[GEO_LOSESCREEN] = MeshBuilder::GenerateOBJ("losescreen", "OBJ//menu.obj");
	meshList[GEO_LOSESCREEN]->textureID = LoadTGA("Image//menu/losescreen.tga");
...
void SceneLose::Update(double dt)
{
	...
	if (Application::IsKeyPressed(VK_RETURN) && pressDelay >= cooldownPressed)
	{
		...
		if (choice == 0)
		{
            //2 represents loading screen.
			Application::ChangeScene(2);

            cout << "Retry Level: " << retryLevel << endl;
            //Change number accordingly to the levels.
            //Tutorial == 4
            //Level 1 == 5
            //Level 2 == 6
            //Level 3 == 7
            //Level 4 == 8
            //Boss == 9
			SceneLoading::ChangeScene(retryLevel);
		}
		// main menu
		else if (choice == 1)
		{
...
void SceneLose::Render()
{
	...
	RenderMeshOnScreen(meshList[GEO_LOSESCREEN], 40, 30, 70, 60);

	if (choice == 0)
	{
		RenderMeshOnScreen(meshList[GEO_ARROW], 35, 28, 50, 50);
	}
	else if (choice == 1)
	{
		RenderMeshOnScreen(meshList[GEO_ARROW], 35, 20, 50, 50);
	}
	else if (choice == 2)
	{
		RenderMeshOnScreen(meshList[GEO_ARROW], 35, 12, 50, 50);
	}
}


void SceneLose::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void SceneLose::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SceneLose::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
	{
		return;
...
void SceneLose::RenderMeshOnScreen(Mesh* mesh, int x, int y, int sizex, int sizey)
{
	glDisable(GL_DEPTH_TEST);
	Mtx44 ortho;
	ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
